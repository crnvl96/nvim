return {
  ...
  opts = function()
    -- Given a list of formatters as arguments, we return the first one  available
    ...
    local function is_web_project(ctx)
      -- Search the project directories recursively until finding a file in the provided list
      return vim.fs.find({ 'package.json' }, { path = ctx.filename, upward = true })[1]
    end

    local function is_json_or_markdown(ctx)
      -- Return true if the currently opened file if one of the provided ones
