...
MiniDeps.now(function()
  ...
  local to_install = vim.tbl_filter(isnt_installed, treesit_langs)
  if #to_install > 0 then require('nvim-treesitter').install(to_install) end

  for _, lang in ipairs(treesit_langs) do
    for _, ft in ipairs(vim.treesitter.language.get_filetypes(lang)) do
      table.insert(treesit_fts, ft)
    end
  end
...
MiniDeps.later(function()
  local build = function(args)
    ...
    local obj = vim.system({ 'npm', 'install', '--prefix', string.format('%s/app', args.path) }, { text = true }):wait()

    if obj.code ~= 0 then
      MiniMisc.put 'An error occurred while building dependencies of markdown-preview.nvim'
    else
      vim.print(vim.inspect(obj))
      MiniMisc.put 'Dependencies of markdown-preview.nvim built successfully'
    end
  end
